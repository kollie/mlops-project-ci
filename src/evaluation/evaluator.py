...
class ModelEvaluator:
    ...
    def _setup_logging(self):
        ...
        try:
            ...
            log_file = log_config.get('file', 'logs/evaluation.log').replace('main.log', 'evaluation.log')
    ...
    def _generate_classification_report(self, y_true: np.ndarray, y_pred: np.ndarray) -> str:
    ...
    def _calculate_roc_curve(self, y_true: np.ndarray, y_pred_proba: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Calculate ROC curve data."""
        try:
            if len(np.unique(y_true)) != 2:
                self.logger.warning("ROC curve calculation only supported for binary classification")
            ...
            self.logger.info(f"ROC curve calculated with {len(thresholds_clean)} points")
    ...
    def _calculate_precision_recall_curve(self, y_true: np.ndarray, y_pred_proba: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Calculate precision-recall curve data."""
        try:
            if len(np.unique(y_true)) != 2:
                self.logger.warning("PR curve calculation only supported for binary classification")
                return None, None, None
    ...
    def evaluate(self, y_true: np.ndarray, y_pred: np.ndarray, 
                y_pred_proba: np.ndarray = None, dataset_name: str = "test") -> Dict[str, Any]:
        ...
        try:
            ...
            if mlflow_enabled:
                with mlflow.start_run():
                    # Calculate metrics
                    self.metrics = self._calculate_basic_metrics(y_true, y_pred, y_pred_proba)
                    self.confusion_matrix = self._calculate_confusion_matrix(y_true, y_pred)
                    self.classification_report = self._generate_classification_report(y_true, y_pred)
                    
                    # Calculate curves if probabilities available
                    if y_pred_proba is not None:
                        self.roc_curve_data = self._calculate_roc_curve(y_true, y_pred_proba)
                        self.pr_curve_data = self._calculate_precision_recall_curve(y_true, y_pred_proba)
    ...
    def get_classification_report(self) -> str:
        ...
        if not self.classification_report:
            raise ValueError("No classification report available. Run evaluate() first.")
    ...
    def get_roc_curve_data(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if self.roc_curve_data is None or self.roc_curve_data[0] is None:
            raise ValueError("No ROC curve data available. Ensure probabilities were provided and it's binary classification.")
    ...
    def get_pr_curve_data(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if self.pr_curve_data is None or self.pr_curve_data[0] is None:
            raise ValueError("No PR curve data available. Ensure probabilities were provided and it's binary classification.")
    ...
    def save_metrics(self, file_path: str = None) -> str:
        ...
        try:
            ...
            if file_path is None:
                metrics_dir = self.config.get('data', {}).get('processed_data_path', 'data/processed')
    ...
    def compare_models(self, other_metrics: Dict[str, float], 
                      primary_metric: str = 'f1_score') -> Dict[str, Any]:
        """Compare this model's metrics with another model."""
        try:
            if not self.metrics:
                raise ValueError("No metrics available for comparison. Run evaluate() first.")
            
            if primary_metric not in self.metrics:
                raise ValueError(f"Primary metric '{primary_metric}' not found in current metrics")
            if primary_metric not in other_metrics:
                raise ValueError(f"Primary metric '{primary_metric}' not found in comparison metrics")
            ...
            self.logger.info(f"Model comparison completed. Current model is {'better' if comparison['is_better'] else 'worse'} on {primary_metric}")
