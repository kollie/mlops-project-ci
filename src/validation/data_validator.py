...
class DataValidator:
    ...
    def __init__(self, config_path: str = "src/config.yaml"):
        ...
        try:
            ...
            self.logger.warning(f"Features configuration not found: {e}. Using basic validation.")
            self.required_columns = []
    ...
    def validate_schema(self, data: pd.DataFrame) -> bool:
        ...
        try:
            if not self.required_columns:
                self.logger.info("No schema requirements defined. Skipping schema validation.")
                return True
            ...
            if 'age_group' in required and 'age_group' not in available and 'age' in available:
                self.logger.warning("Expected 'age_group' not found. Using 'age' as acceptable substitute.")
    ...
    def validate_data_types(self, data: pd.DataFrame) -> bool:
        ...
        try:
            if 'features' not in self.config:
                self.logger.info("No data type requirements defined. Skipping type validation.")
                return True
            ...
            for col in self.config['features'].get('categorical_features', []):
                if col in data.columns and not (pd.api.types.is_string_dtype(data[col]) or 
    ...
    def validate_missing_values(self, data: pd.DataFrame, threshold: float = 0.5) -> bool:
        ...
        try:
            ...
            if not high_missing.empty:
                self.logger.error(f"Columns with >{threshold*100}% missing values: {list(high_missing.index)}")
                self.report['high_missing_columns'] = list(high_missing.index)
                return False
                
            self.logger.info(f"Missing value validation passed. Max missing ratio: {missing_ratio.max():.2%}")
    ...
    def validate_target_distribution(self, data: pd.DataFrame, min_class_ratio: float = 0.01) -> bool:
        """Validate target variable distribution."""
        try:
            if 'features' not in self.config or 'target_column' not in self.config['features']:
                self.logger.info("No target column defined. Skipping target distribution validation.")
                return True
            ...
            if (dist < min_class_ratio).any():
                self.logger.error(f"Target class imbalance detected. Min class ratio: {dist.min():.2%}")
                self.report['target_imbalance'] = True
                return False
                
            self.logger.info(f"Target distribution validation passed. Classes: {dist.to_dict()}")
    ...
    def validate_all(self, data: pd.DataFrame, **kwargs) -> Dict[str, bool]:
        ...
        results = {
            ...
            "missing_values": self.validate_missing_values(data, kwargs.get('missing_threshold', 0.5)),
            "target_distribution": self.validate_target_distribution(data, kwargs.get('min_class_ratio', 0.01))
        }
    ...
    def validate_and_clean(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            self.report['columns_dropped'] = self.report['initial_columns'] - self.report['final_columns']
            ...
            self.logger.info(f"   Rows: {self.report['initial_rows']} → {self.report['final_rows']} ({-rows_change:+d})")
            self.logger.info(f"   Columns: {self.report['initial_columns']} → {self.report['final_columns']} ({-cols_change:+d})")
    ...
    def _handle_missing_values(self, data: pd.DataFrame, strategy: str = 'drop') -> pd.DataFrame:
        ...
        if strategy == 'drop':
            ...
            if rows_with_missing > 0:
                ...
                for col in data.columns:
                    ...
                    if missing_count > 0:
                        ...
                        self.logger.info(f"  Column '{col}': {missing_count} missing values ({missing_pct:.1f}%)")
            ...
            if rows_dropped > 0:
                self.logger.info(f"✂️  Dropped {rows_dropped} rows with missing values (strategy: {strategy})")
                self.logger.info(f"   Rows: {rows_before} → {len(data_cleaned)} ({rows_dropped} removed)")
            else:
            ...
            threshold = 0.5
            high_missing_cols = missing_summary[missing_summary > len(data) * threshold].index
            
            if len(high_missing_cols) > 0:
                self.logger.warning(f"🗑️  Dropping {len(high_missing_cols)} columns with >{threshold*100}% missing values:")
                for col in high_missing_cols:
                    missing_count = missing_summary[col]
                    missing_pct = missing_ratios[col] * 100
                    self.logger.warning(f"   - Column '{col}': {missing_count}/{len(data)} missing ({missing_pct:.1f}%)")
                
                data_cleaned = data.drop(columns=high_missing_cols)
                self.logger.info(f"   Columns: {cols_before} → {len(data_cleaned.columns)} ({len(high_missing_cols)} removed)")
                ...
                self.logger.info(f"✅ No columns dropped - no columns exceed {threshold*100}% missing threshold")
                data_cleaned = data.copy()
                
        else:
            # For now, default to dropping rows
            self.logger.warning(f"⚠️  Unknown missing value strategy '{strategy}'. Using 'drop' as fallback.")
            ...
            if rows_dropped > 0:
                self.logger.info(f"✂️  Dropped {rows_dropped} rows with missing values (fallback strategy)")
