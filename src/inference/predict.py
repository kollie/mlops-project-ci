...
class ModelPredictor:
    ...
    def _setup_logging(self):
        ...
        try:
            ...
            log_file = log_config.get('file', 'logs/inference.log').replace('main.log', 'inference.log')
    ...
    def load_model(self, model_path: str) -> None:
        ...
        try:
            ...
            if isinstance(model_data, dict):
                ...
                self.feature_engineer = None
                self.model_metadata = {}
                self.logger.warning("Model file doesn't contain feature engineer. Manual preprocessing may be required.")
        ...
        try:
            ...
            for col in numerical_cols:
                if data[col].isnull().any():
                    self.logger.warning(f"Found NaN values in column '{col}'. These may cause prediction errors.")
                if np.isinf(data[col]).any():
                    raise ValueError(f"Found infinite values in column '{col}'. Please clean the data.")
    ...
    def _preprocess_data(self, data: pd.DataFrame, target_col: str = 'readmitted') -> pd.DataFrame:
        """Preprocess data for prediction."""
        try:
            if self.feature_engineer is None:
                self.logger.warning("No feature engineer available. Using raw data for prediction.")
            ...
            X_processed, y_dummy = self.feature_engineer.transform(data_with_target, target_col=target_col)
    ...
    def predict(self, data: pd.DataFrame, preprocess: bool = True) -> np.ndarray:
        ...
        try:
            ...
            self.logger.info(f"Prediction distribution: {dict(zip(*np.unique(predictions, return_counts=True)))}")
    ...
    def predict_with_confidence(self, data: pd.DataFrame, confidence_threshold: float = 0.8, 
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Could not calculate probabilities: {e}. Returning predictions only.")
                results = {
                    'predictions': predictions,
                    'probabilities': None,
            ...
            self.logger.info(f"  High confidence predictions: {results['high_confidence_count']}/{len(predictions)}")
            self.logger.info(f"  Low confidence predictions: {results['low_confidence_count']}/{len(predictions)}")
    ...
    def predict_batch(self, data: pd.DataFrame, batch_size: int = 1000, 
                     save_path: str = None) -> Dict[str, Any]:
        """Process large datasets in batches."""
        try:
            self.logger.info(f"🔄 Processing {len(data)} samples in batches of {batch_size}...")
            ...
            for i in range(n_batches):
                ...
                try:
                    batch_probabilities = self.predict_proba(batch_data, preprocess=True)
            ...
            final_probabilities = np.array(all_probabilities) if all_probabilities else None
            ...
            results = {
                ...
                'prediction_distribution': dict(zip(*np.unique(final_predictions, return_counts=True)))
            }
    ...
    def save_predictions(self, results: Dict[str, Any], save_path: str) -> str:
        ...
        try:
            ...
            save_data = {
                ...
                'prediction_distribution': convert_numpy_types(results.get('prediction_distribution', {})),
            ...
            if 'confidence_scores' in results and results['confidence_scores'] is not None:
                save_data['confidence_scores'] = results['confidence_scores'].tolist()
                save_data['confidence_stats'] = convert_numpy_types(results.get('confidence_stats', {}))
    ...
    def _write_report(self):
        ...
        try:
            ...
            self.report['prediction_history'] = convert_numpy_types(self.prediction_history)
