...
class Preprocessor:
    ...
    def _setup_logging(self):
        ...
        try:
            ...
            log_file = log_config.get('file', 'logs/preprocessing.log').replace('main.log', 'preprocessing.log')
    ...
    def _get_feature_columns(self, data: pd.DataFrame) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            numerical_features = [col for col in config_numerical 
                                if col in data.columns and col not in drop_columns]
            categorical_features = [col for col in config_categorical 
                                  if col in data.columns and col not in drop_columns]
    ...
    def _create_preprocessing_pipeline(self, numerical_features: List[str], 
        ...
        try:
            ...
            if numerical_features:
                self.logger.info(f"Adding numerical pipeline for {len(numerical_features)} features")
            ...
            if categorical_features:
                self.logger.info(f"Adding categorical pipeline for {len(categorical_features)} features")
            ...
            if not transformers:
                raise ValueError("No valid features available for preprocessing pipeline")
    ...
    def _select_features(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if zero_var_mask.any():
                ...
                self.logger.warning(f"Found {zero_var_count} zero variance features, removing them")
                X = X[:, ~zero_var_mask]
            ...
            if k >= X.shape[1]:
                self.logger.info("All remaining features selected (k >= remaining features)")
                return X
            ...
            selected_features = [name for name, selected in zip(self._feature_names, selected_mask) if selected]
            
            self.logger.info(f"✅ Selected {len(selected_features)} features from {X.shape[1]} total")
    ...
    def _update_feature_names_after_fit(self, numerical_features: List[str], 
        ...
        try:
            ...
            self.logger.info(f"Updated feature names: {len(self._feature_names)} total features")
    ...
    def get_feature_names(self) -> List[str]:
        ...
        selected_mask = self.feature_selector.get_support()
        return [name for name, selected in zip(self._feature_names, selected_mask) if selected]
