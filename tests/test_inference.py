    ...
    def transform(self, data, target_col="readmitted"):
        ...
        if len(features.columns) != 3:
            features = pd.DataFrame(
                np.random.random((len(data), 3)), columns=["feat1", "feat2", "feat3"]
            )
        else:
...
class TestModelPredictor:
    ...
    def test_predictor_initialization_with_model(
        self, sample_config, sample_model_file
    ):
        """Test ModelPredictor initialization with model loading."""
        predictor = ModelPredictor(
            config_path=sample_config, model_path=sample_model_file
        )
    ...
    def test_validate_input_data_empty(self, model_predictor):
        ...
        with pytest.raises(
            ValueError, match="Cannot make predictions on empty DataFrame"
        ):
    ...
    def test_validate_input_data_infinite_values(self, model_predictor):
        ...
        data_with_inf = pd.DataFrame(
            {"feature1": [1.0, 2.0, np.inf], "feature2": [1.0, 2.0, 3.0]}
        )
    ...
    def test_predict_proba_success(
        self, model_predictor, sample_model_file, sample_data
    ):
    ...
    def test_predict_proba_model_without_support(
        self, model_predictor, sample_model_file, sample_data
    ):
        ...
        with pytest.raises(
            ValueError, match="does not support probability predictions"
        ):
            model_predictor.predict_proba(sample_data)

    # ---------- Confidence Prediction Tests ----------

    def test_predict_with_confidence_success(
        self, model_predictor, sample_model_file, sample_data
    ):
        """Test successful confidence prediction."""
        model_predictor.load_model(sample_model_file)

        results = model_predictor.predict_with_confidence(
            sample_data, confidence_threshold=0.7
        )
        ...
        assert results["high_confidence_count"] + results[
            "low_confidence_count"
        ] == len(sample_data)

    def test_predict_with_confidence_fallback(
        self, model_predictor, sample_model_file, sample_data
    ):
    ...
    def test_predict_batch_with_save(
        self, model_predictor, sample_model_file, temp_dir
    ):
        ...
        data = pd.DataFrame(
            np.random.random((15, 3)), columns=["feature1", "feature2", "feature3"]
        )
    ...
    def test_predict_batch_success(self, model_predictor, sample_model_file):
        ...
        large_data = pd.DataFrame(
            np.random.random((25, 3)), columns=["feature1", "feature2", "feature3"]
        )
    ...
    def test_prediction_history_management(
        self, model_predictor, sample_model_file, sample_data
    ):
    ...
    def test_preprocess_data_with_feature_engineer(
        self, model_predictor, sample_model_file, sample_data
    ):
    ...
    def test_preprocess_data_without_feature_engineer(
        self, model_predictor, sample_data
    ):
    ...
    def test_report_generation(
        self, model_predictor, sample_model_file, sample_data, temp_dir
    ):
