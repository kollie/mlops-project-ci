...
class TestModelTrainer:
    ...
    def test_fit_mismatched_lengths(self, model_trainer):
        ...
        with pytest.raises(
            ValueError, match="Training features and labels must have same length"
        ):
            model_trainer.fit(X, y)

    def test_fit_with_categorical_target(
        self, model_trainer, sample_data_categorical, temp_dir
    ):
    ...
    def test_get_feature_importance_logistic_regression(
        self, sample_config, sample_data
    ):
