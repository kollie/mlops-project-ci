...
class TestDataValidator:
    ...
    def test_validate_schema_age_fallback(self, validator, sample_data_with_age_fallback):
    ...
    def test_validate_target_distribution_fails(self, validator, sample_data):
        ...
        result = validator.validate_target_distribution(sample_data, min_class_ratio=0.05)
        assert result is False
    ...
    def test_missing_value_strategies(self, validator, sample_data):
        ...
        # Test drop strategy
        clean_data_drop = validator.validate_and_clean(sample_data.copy(), missing_strategy='drop')
        assert len(clean_data_drop) == len(sample_data) - 11  # 11 rows with missing values
        
        # Test unknown strategy (should fallback to drop)
        clean_data_unknown = validator.validate_and_clean(sample_data.copy(), missing_strategy='unknown')
