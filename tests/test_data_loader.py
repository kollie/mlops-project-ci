...
class TestDataLoader:
    ...
    def test_split_data_success(self, sample_config, sample_data):
        ...
        assert 0.15 <= test_ratio <= 0.25, f"Test ratio {test_ratio:.2f} not in expected range"
        
        # Validation should be approximately 16% (20% of remaining 80%) (±5%)
        assert 0.11 <= val_ratio <= 0.25, f"Validation ratio {val_ratio:.2f} not in expected range"
        
        # Train should be the majority (±5%)
        assert 0.55 <= train_ratio <= 0.75, f"Train ratio {train_ratio:.2f} not in expected range"
