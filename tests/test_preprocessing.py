...
class TestPreprocessor:
    ...
    def sample_config(self, temp_dir):
        ...
        config = {
            'data': {
                ...
                'validation_data_path': os.path.join(temp_dir, "processed", "validation.csv"),
    ...
    def sample_data(self):
        ...
        for key, values in data.items():
            assert len(values) == 100, f"Column {key} has {len(values)} values, expected 100"
    ...
    def sample_data_with_missing(self):
        ...
        data = {
            ...
            'readmitted': (readmitted_pattern * 50)[:100],  # Repeat and truncate to exactly 100
        }
    ...
    def test_get_feature_columns(self, preprocessor, sample_data):
        ...
        numerical_features, categorical_features = preprocessor._get_feature_columns(sample_data)
    ...
    def test_get_feature_columns_missing_columns(self, preprocessor):
        ...
        numerical_features, categorical_features = preprocessor._get_feature_columns(data)
    ...
    def test_create_preprocessing_pipeline(self, preprocessor, sample_data):
        ...
        numerical_features, categorical_features = preprocessor._get_feature_columns(sample_data)
        pipeline = preprocessor._create_preprocessing_pipeline(numerical_features, categorical_features)
    ...
    def test_fit_transform_custom_target(self, preprocessor, sample_data):
        ...
        X_processed, y_processed = preprocessor.fit_transform(sample_data, target_col='target')
    ...
    def test_logging_output(self, preprocessor, sample_data, temp_dir):
        ...
        try:
            ...
            assert len(final_logs.strip()) > 0, "No logs were captured during preprocessing"
            
            # Just check that it contains some key words related to preprocessing
            log_lower = final_logs.lower()
            preprocessing_keywords = ['preprocessing', 'fitting', 'encoding', 'pipeline', 'features']
            
            found_keywords = [keyword for keyword in preprocessing_keywords if keyword in log_lower]
            assert len(found_keywords) >= 2, f"Expected at least 2 preprocessing-related keywords, found: {found_keywords}"
